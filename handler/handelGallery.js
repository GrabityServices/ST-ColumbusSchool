...
async function setGalleryForm(req, res) {
  if (req.body.uploaded == true) {
    const gallery = await Gallery.findByIdAndUpdate(req.params.id, {
      ...
      if (gallIs.img && gallIs.img !== "/adminAvt/defaultAvt.png") {
        try {
          const imagePath = path.join(__dirname, "../assets", gallIs.img); // Adjust based on your storage setup
...
async function handelNewGalleryForm(req, res) {
  if (req.body.uploaded) {
    const { title, EventDate, filename } = req.body;
    ...
    // console.log(newGal);
    return res.redirect("/stcolumbus/jaj/ekdara/admin#gallery");
  } else {
    try {
      const imagePath = path.join(
        __dirname,
